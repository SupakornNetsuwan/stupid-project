...
import { randomFromServer } from "~/routes";
...
const Item = component$<{ memo: Memo; deleteMemo: (memo: Memo) => void }>(({ memo, deleteMemo }) => {
  ...
  const isForgetCSS = randomFromServer(20);

  return (
    <div
      ref={element}
      class={`${isForgetCSS ? "font-mono" : "shadow border rounded p-4 "} w-full space-y-3`}
    >
      <div class="flex items-center justify-between">
        <h3 class={`${isForgetCSS ? "" : "text-primary font-medium text-xl"}  `}>{memo.title}</h3>
        {isForgetCSS ? (
          "bin.svg"
        ) : (
          <HiTrashOutline
