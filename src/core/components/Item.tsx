...
const Item = component$<{ memo: Memo; deleteMemo: (memo: Memo) => void }>(
  ({ memo, deleteMemo }) => {
    const element = useSignal<HTMLElement>();
    useVisibleTask$(() => {
      const [title, description, date] =
        [element.value?.querySelector("div")].concat(
          ...(element.value?.querySelectorAll("p") || [])
        ) || [];
      ...
      timeline([
        [
          element.value!,
          { y: [30, 0] },
          { duration: 0.5, easing: spring({ mass: 0.8, velocity: 4 }) },
        ],
        [title, { y: [10, 0], opacity: [0, 1] }, { at: 0.5 }],
        [
          description,
          { y: [10, 0], opacity: [0, 1] },
          { at: 0.6, easing: spring({ mass: 0.8, velocity: 8 }) },
        ],
        [
          date,
          { y: [10, 0], opacity: [0, 1] },
          { at: 0.7, easing: spring({ mass: 0.8, velocity: 10 }) },
        ],
      ]);
    });
