...
import { stagger, timeline, spring, animate } from "motion";
...
export default component$<{ isMouseOver: boolean }>(({ isMouseOver }) => {
  ...
  useVisibleTask$(({ track }) => {
    const hoverAnimation = timeline(
      [
        ["#title", { y: [10, 0, -2, 6, -4], x: [10, 0, 4, -2, 4] }, { duration: 0.3 }],
        [
          "#title > span",
  ...
  return (
    <div id="hero" class="flex justify-center items-center py-12 w-full">
      <div class="relative flex items-end justify-end">
        <div ref={outputRef} id="title" class="text-[20vw] md:text-9xl font-bold">
          <span>M</span>
          <span>e</span>
          <span>m</span>
          <span>o</span>
        ...
        <p
          id="shit"
          style={{ textShadow: " -1px -1px 0 white, 1px -1px 0 black, -1px 1px 0 white, 1px 1px 0 white;" }}
